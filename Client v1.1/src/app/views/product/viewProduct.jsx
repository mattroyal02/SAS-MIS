...
const ProductView = () => {
  const { user } = useAuth();
  const { customerId, id, productId } = useParams();
  ...
  const {
    ...
    url: `http://localhost:4050/products/${productId}`,
  });

  const getData = async () => {
    const { data } = await axios.get(
      `http://localhost:4050/products/${productId}`,
      {
        params: { search },
      }
    );
          ...
          <Container maxWidth="lg">
            <Grid container spacing={3}>
              <Grid item lg={10} md={10} xs={8}>
                <ProjectProfileDetails name={project.data.name} />
              </Grid>
            </Grid>
          </Container>
        )}
